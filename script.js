...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleTabClick() {
        ...
        // If on mobile, gently center the tab using horizontal scroll only
        if (deviceType !== 'desktop') {
            const tabsContainer = this.closest('.tabs');
            if (tabsContainer) {
                const desiredScroll = this.offsetLeft - (tabsContainer.clientWidth - this.offsetWidth) / 2;
    ...
    function initTabs() {
        ...
        if (tabsContainer && deviceType !== 'desktop') {
            ...
            if (activeTab) {
                setTimeout(() => {
                    const desiredScroll = activeTab.offsetLeft - (tabsContainer.clientWidth - activeTab.offsetWidth) / 2;
                    tabsContainer.scrollTo({
                        left: Math.max(0, desiredScroll),
                        behavior: 'smooth'
                    });
